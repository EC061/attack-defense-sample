...
class APIClient:
    ...
    def __init__(
        self, client: OpenAI, model_name: str, service_tier: Optional[str] = None
    ):
        """
    ...
    def create_message_with_image(
        image_path: str, prompt: str, use_url: bool = False
    ) -> List[Dict[str, Any]]:
        """
    ...
    def create_message_with_multiple_images(
        image_paths: List[str], prompt: str, use_url: bool = False
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_cost(
        ...
        """
        Calculate cost using service-tier-aware pricing calculator.
    ...
    def generate_with_retry(
        ...
        Notes:
            Prints the total retry count once after a successful completion.
            TTFT and pp_per_sec exclude retry sleep delays.
        """
        retry_count = 0
        total_retry_sleep_time = 0.0
        ...
        while True:
            ...
            try:
                content, metrics = self._stream_completion(
                    messages, response_format, temperature, presence_penalty, max_tokens
                )

                # Calculate retry time cost (time from first request to before successful one)
                retry_time_cost = 0.0
                if retry_count > 0:
                    # Retry time = time before successful attempt started
                    retry_time_cost = attempt_start_time - overall_start_time
                    # total_time should reflect the entire operation including retries
                    metrics.total_time = time.time() - overall_start_time
                    # TTFT remains as measured for the successful attempt (doesn't include retry sleeps)
                    # Note: TTFT already excludes retry sleep time since each attempt has its own timer

                print(f"Total retries: {retry_count}", end="")
                if retry_count > 0:
                    print(
                        f", Retry time cost: {retry_time_cost:.3f}s, Total time: {metrics.total_time:.3f}s"
                    )
                else:
