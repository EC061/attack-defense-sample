...
class MaterialsDescriptionGenerator:
    ...
    def generate_description(
        ...
        """
        messages = APIClient.create_message_with_image(
            image_path, prompt, use_url=use_url
        )
    ...
    def _process_individual_task(
        ...
        try:
            ...
            content_text = (
                result.content if isinstance(result, GenerationResult) else result
            )
            try:
                parsed = json.loads(content_text)
            except Exception as parse_err:
                raise ValueError(
                    f"Failed to parse JSON: {parse_err}; content: {content_text}"
                )
            ...
            if metrics:
                ...
                print(
                    f"  ✓ TTFT: {metrics.ttft:.3f}s, Generation time: {metrics.generation_time:.3f}s, TG/sec: {metrics.tg_per_sec:.2f}, PP/sec: {metrics.pp_per_sec:.2f}"
                )
    ...
    def _process_batch_task(
        ...
        try:
            ...
            messages = APIClient.create_message_with_multiple_images(
                filtered_image_paths, batch_prompt, use_url=False
            )
            ...
            content, metrics = self.api_client.generate_with_retry(
                messages=messages, response_format=response_format, verbose=True
            )
            ...
            try:
                ...
                raise ValueError(
                    f"Failed to parse JSON: {parse_err}; content: {content}"
                )
    ...
    def process_materials_db(
        ...
        if materials_dir is None:
            materials_dir = self.cfg.get("paths", {}).get(
    ...
    def _print_summary(
        ...
        print("Total batches: 1 (single inference)")
