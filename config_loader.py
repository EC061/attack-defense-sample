...
def as_env_dict(cfg: Dict[str, Any], model_type: str = "vlm") -> Dict[str, str]:
    ...
    if model_type == "llm":
        env["MODEL_NAME"] = str(
            _deep_get(
                cfg, "model.llm.name", "QuantTrio/Qwen3-235B-A22B-Thinking-2507-AWQ"
            )
        )
    else:
        env["MODEL_NAME"] = str(
            _deep_get(cfg, "model.vlm.name", "Qwen/Qwen3-VL-30B-A3B-Instruct")
        )
...
def main() -> None:
    ...
    if model_type not in ("vlm", "llm"):
        print(
            f"Error: Invalid model type '{model_type}'. Must be 'vlm' or 'llm'.",
            file=sys.stderr,
        )
        sys.exit(1)
